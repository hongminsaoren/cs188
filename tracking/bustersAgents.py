...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for each_ghost in livingGhostPositionDistributions:
            print('the', i, 'prob distribution is', each_ghost)
            i += 1
        for a in gameState.getLegalPacmanActions():
            return a
        """
        min_dis = float('inf')
        closest_pos = pacmanPosition
        for each_ghost in livingGhostPositionDistributions:
            most_likely_pos = max(each_ghost, key=each_ghost.get)
            if self.distancer.getDistance(pacmanPosition, most_likely_pos) < min_dis:
                min_dis = self.distancer.getDistance(pacmanPosition, most_likely_pos)
                closest_pos = most_likely_pos
        next_dis = float('inf')
        best_act = None
        for action in legal:
            next_pos = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(next_pos, closest_pos) < next_dis:
                best_act = action
                next_dis = self.distancer.getDistance(next_pos, closest_pos)
        return best_act
        "*** END YOUR CODE HERE ***"
