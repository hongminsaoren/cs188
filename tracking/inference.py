...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict,
        ...
        for eliminate_var in eliminationOrder:
            currentFactorsList, joined_factor = joinFactorsByVariable(currentFactorsList, eliminate_var)
            if len(joined_factor.unconditionedVariables()) > 1:
                new_factor = eliminate(joined_factor, eliminate_var)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        self.normalize()
        dict_lst = list(self.items())
        a = random.random()
        idx = 0
        while a > 0:
            a -= dict_lst[idx][1]
            idx += 1
        return dict_lst[idx - 1][0]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            self.beliefs[p] *= self.getObservationProb(observation, gameState.getPacmanPosition(), p,
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for p in new_beliefs:
            new_beliefs[p] = 0
        for old_pos in self.allPositions:
            new_pos = self.getPositionDistribution(gameState, old_pos)
            for p, prob in new_pos.items():
                new_beliefs[p] += prob * self.beliefs[old_pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        flag = False
        for particle in self.particles:
            distribution[particle] += self.getObservationProb(observation, gameState.getPacmanPosition(), particle,
                                                              self.getJailPosition())
            if distribution[particle] != 0:
                flag = True
        if flag:
            self.particles = []
